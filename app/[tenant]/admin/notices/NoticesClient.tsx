...
import { Pencil, Trash2, Megaphone, Calendar, Search } from "lucide-react";
...
export default function NoticesClient({ initialNotices, tenant }: { initialNotices: any[], tenant: string }) {
  ...
  const filteredNotices = notices.filter(n => 
    ...
    n.targetRole?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4 bg-secondary/5 border border-border/40 p-2 rounded-2xl w-full">
        <div className="relative flex-1">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <input 
            type="text"
            placeholder="Search notices by title, content, or target role..."
            value={searchTerm}
      ...
      {filteredNotices.length > 0 ? (
        ...
        <Card className="border-border/40 bg-card/50 backdrop-blur-md p-16 flex flex-col items-center justify-center text-center gap-4 rounded-[2rem]">
          ...
          <div>
            <h3 className="text-xl font-display font-black text-foreground">No notices found</h3>
            <p className="text-muted-foreground text-sm mt-1 max-w-sm mx-auto">
