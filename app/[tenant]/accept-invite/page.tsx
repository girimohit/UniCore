    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-background">
        <ErrorCard 
          ...
          action={<Link href={`/${tenant}/login`} className="btn-primary px-6 py-2.5 rounded-xl font-bold">Go to Login</Link>}
        />
      </div>
    );
  }
...
function ErrorCard({ icon, title, message, action }: { icon: React.ReactNode, title: string, message: string, action?: React.ReactNode }) {
  return (
