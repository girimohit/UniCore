...
import { CheckSquare, Calendar, Users, Save, Filter, Loader2, CheckCircle } from "lucide-react";
...
export default function AttendanceManager({ subjects, students, periods }: AttendanceManagerProps) {
      ...
      <div className="xl:col-span-1 space-y-5 glass rounded-3xl p-8 border border-border/50 shadow-xl shadow-primary/5 h-fit">
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-foreground tracking-wide uppercase flex items-center gap-2">
            ...
            Academic Period
            <span className="text-[10px] font-normal text-muted-foreground normal-case ml-1">(optional)</span>
          </label>
          <select
            value={periodId}
        ...
        {error && (
          <p className="text-xs font-semibold text-destructive bg-destructive/10 rounded-xl px-4 py-2">{error}</p>
        )}
        ...
        >
          {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Save className="h-5 w-5" /> Save Records</>}
        </button>
      </div>
      ...
      <div className="xl:col-span-2 glass border border-border/50 rounded-3xl shadow-xl shadow-primary/5 flex flex-col overflow-hidden max-h-[70vh]">
        <div className="p-6 border-b border-border/50 bg-background/30 flex flex-col sm:flex-row justify-between items-center gap-4 backdrop-blur-md">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => setStatuses(Object.fromEntries(students.map(s => [s.id, "ABSENT"])))}
          ...
          {students.length === 0 ? (
            ...
            <table className="w-full text-left text-sm text-foreground">
              <thead className="bg-muted/50 text-muted-foreground text-xs uppercase font-extrabold tracking-widest sticky top-0 z-10 backdrop-blur-xl">
                <tr>
                  <th className="px-6 py-4 rounded-tl-xl border-b border-border/50">Roll No</th>
                  <th className="px-6 py-4 border-b border-border/50">Name</th>
                  <th className="px-6 py-4 rounded-tr-xl border-b border-border/50 text-center">Status</th>
                </tr>
              </thead>
                      ...
                      <div className="flex justify-center gap-3">
                        <label className="flex items-center justify-center cursor-pointer relative">
                          <input
                            ...
                            value="PRESENT"
                            checked={!statuses[student.id] || statuses[student.id] === "PRESENT"}
