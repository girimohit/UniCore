...
export default async function StudentProfilePage({ params }: { params: Promise<{ tenant: string }> }) {
  ...
  const initials = student.user.identifier.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid gap-8 md:grid-cols-3">
         ...
         <div className="md:col-span-1 space-y-6">
            <Card className="border border-slate-200 shadow-sm overflow-hidden text-center group text-black bg-white">
                <div className="h-32 bg-slate-900 relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-indigo-800 to-slate-900 opacity-80"></div>
                </div>
                <div className="px-6 pb-6 relative">
                    <div className="w-24 h-24 rounded-full bg-white p-1 mx-auto -mt-12 relative cursor-pointer shadow-md">
                        <div className="w-full h-full rounded-full bg-slate-100 overflow-hidden flex items-center justify-center text-indigo-600 font-black text-3xl group-hover:bg-slate-200 transition-colors">
                            {initials}
                        </div>
                    ...
                    <div className="mt-4 space-y-1">
                        ...
                        <p className="text-slate-500 text-sm font-medium">Computer Science • Semester 4</p>
                        <div className="pt-2">
                             <span className="text-[10px] font-black text-indigo-700 bg-indigo-50 border border-indigo-100 py-1 px-3 rounded-full inline-block uppercase tracking-widest">
                                ID: {student.roll_number || 'STU-2024'}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <Card className="border border-slate-200 shadow-sm overflow-hidden text-black bg-white">
                <CardContent className="p-2 space-y-1">
                    <button className="w-full text-left px-4 py-3 rounded-lg hover:bg-slate-50 text-sm font-bold text-slate-600 hover:text-indigo-600 transition-colors flex items-center gap-3">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input defaultValue={student.user.identifier} className="pl-9 border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" />
                            </div>
                        </div>
                         <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input defaultValue={student.roll_number || ''} disabled className="pl-9 bg-slate-50 border-slate-200 cursor-not-allowed" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Email Address</label>
                         <div className="relative">
                            ...
                            <Input defaultValue={student.user.email} className="pl-9 border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" />
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Phone Number</label>
                         <div className="relative">
                            ...
                            <Input defaultValue="+91 98765 43210" className="pl-9 border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" />
                        </div>
                    </div>

                     <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Campus Address</label>
                         <div className="relative">
                            <MapPin className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
                            <Input defaultValue="Hostel Block A, Room 302, University Campus" className="pl-9 border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" />
                        </div>
                    </div>
